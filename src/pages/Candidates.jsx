...
// export default function Candidates() {
//   const [resumes, setResumes] = useState([]);
//   const [jds, setJds] = useState([]); // <-- State to hold your JDs
//   const [selectedJdId, setSelectedJdId] = useState(""); // <-- State for the dropdown
//   const [loading, setLoading] = useState(false);
  
//   // Manual Search and Filter States
//   const [searchTerm, setSearchTerm] = useState(''); 
//   const [skillsInput, setSkillsInput] = useState(''); 
//   const [minExperience, setMinExperience] = useState('');

//   // 1. Fetch the Job Descriptions on component load
//   useEffect(() => {
...
//     };
//     fetchDropdownData();
//     fetchResumes(); // Load candidates initially too
//   }, []);
...
//     // --- NEW: Determine which skills to use (JD vs Manual) ---
...
//     if (selectedJd) {
//       // If a JD is selected, OVERRIDE manual inputs with the database JD requirements
...
//     } else {
//       // Otherwise, use the manual text boxes
...
//       let experienceScore = 100;
//       if (requiredExpNum > 0) {
//         experienceScore = resume.total_years_experience >= requiredExpNum
//           ? 100
//           : Math.round((resume.total_years_experience / requiredExpNum) * 100);
//       }
...
//           {/* NEW: Automated JD Dropdown */}
//           <div className="w-1/3">
//             <select 
//               className="w-full border border-indigo-300 bg-indigo-50 text-indigo-900 p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-medium cursor-pointer"
//               value={selectedJdId}
//               onChange={(e) => setSelectedJdId(e.target.value)}
//             >
//               <option value="">-- Score using Manual Filters --</option>
//               {jds.map(jd => (
//                 <option key={jd.id} value={jd.id}>Match to: {jd.title}</option>
//               ))}
//             </select>
//           </div>
//         </div>
        
//         {/* Manual Filters (Disabled if a JD is selected) */}
...
//         <button 
//           onClick={fetchResumes}
//           className="bg-gray-900 text-white px-5 py-2.5 rounded-lg hover:bg-gray-800 transition shadow-sm font-medium w-full md:w-auto"
//         >
//           {loading ? 'Analyzing...' : (selectedJdId ? 'Run Automated JD Match' : 'Run Manual Scoring Engine')}
//         </button>
//       </div>
...
//                   const isMatched = resume.matchedSkills?.includes(skill.toLowerCase());
//                   return (
//                     <span key={skill} className={`text-sm px-3 py-1 rounded-full ${isMatched ? 'bg-blue-100 text-blue-800 font-semibold border border-blue-200' : 'bg-gray-100 text-gray-600'}`}>
//                       {skill}
//                     </span>
//                   );
//                 })}
...
//                   <div className="flex flex-wrap gap-2">
//                     {resume.missingSkills.map(missingSkill => (
//                       <span key={missingSkill} className="text-xs px-2 py-1 rounded-md bg-red-50 text-red-600 border border-red-100">
//                         {missingSkill}
//                       </span>
//                     ))}
//                   </div>
...
export default function Candidates() {
  ...
  const fetchResumes = async () => {
    ...
    const scoredResumes = (data || []).map(resume => {
      ...
      // 1. Calculate Skill Matches using Partial Substring Match
      if (requiredSkillsArray.length > 0) {
      ...
      // 4. Overall Weight: 70% Skills, 30% Experience
      const overallScore = Math.round((skillMatchScore * 0.7) + (experienceScore * 0.3));
