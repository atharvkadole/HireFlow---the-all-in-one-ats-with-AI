...
export default function Analytics() {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const [
        { count: totalCandidates, error: countError },
        { count: jdCount, error: jdError },
      ] = await Promise.all([
        supabase
          .from("resumes")
          .select("*", {
      ...
      // Average Experience
      const totalExp = resumes.reduce(
        (sum, resume) =>
          sum + (Number(resume.total_years_experience) || 0),
        0
      );
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="bg-white h-24 rounded-xl shadow-sm border border-gray-100 animate-pulse"
          />
        ))}
      </div>
    );
